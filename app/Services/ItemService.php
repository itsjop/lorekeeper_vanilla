...
class ItemService extends Service {
  ...
  private function populateCategoryData($data, $category = null) {
    ...
    isset($data['is_character_owned']) && $data['is_character_owned']
      ? $data['is_character_owned']
      : ($data['is_character_owned'] = 0);
    isset($data['character_limit']) && $data['character_limit']
  ...
  public function deleteItemCategory($category) {
    ...
    try {
      ...
      if (Item::where('item_category_id', $category->id)->exists()) {
        throw new \Exception(
          'An item with this category exists. Please change its category first.'
        );
      }
  ...
  public function createItem($data, $user) {
    ...
    try {
      ...
      if (
        isset($data['item_category_id']) &&
        $data['item_category_id'] &&
        !ItemCategory::where('id', $data['item_category_id'])->exists()
      ) {
      ...
      $item->update([
        'data' => json_encode([
          ...
          'resell' => isset($data['currency_quantity'])
            ? [$data['currency_id'] => $data['currency_quantity']]
            : null
  ...
  private function populateData($data, $item = null) {
    ...
    if (
      !isset($data['is_released']) &&
      Config::get('lorekeeper.extensions.item_entry_expansion.extra_fields')
    ) {
      $data['is_released'] = 0;
    } else {
      $data['is_released'] = 1;
  ...
  public function deleteItem($item) {
    ...
    try {
      ...
      if (
        DB::table('user_items')
          ->where([['item_id', '=', $item->id], ['count', '>', 0]])
          ->exists()
      ) {
        throw new \Exception(
          'At least one user currently owns this item. Please remove the item(s) before deleting it.'
        );
      }
      if (
        ...
        throw new \Exception(
          'At least one character currently owns this item. Please remove the item(s) before deleting it.'
        );
      }
      if (
        DB::table('loots')
          ->where('rewardable_type', 'Item')
          ->where('rewardable_id', $item->id)
          ->exists()
      ) {
        throw new \Exception(
          'A loot table currently distributes this item as a potential reward. Please remove the item before deleting it.'
        );
      }
      if (
        DB::table('prompt_rewards')
          ->where('rewardable_type', 'Item')
          ->where('rewardable_id', $item->id)
          ->exists()
      ) {
        throw new \Exception(
          'A prompt currently distributes this item as a reward. Please remove the item before deleting it.'
        );
      }
      if (DB::table('shop_stock')->where('item_id', $item->id)->exists()) {
        throw new \Exception(
          'A shop currently stocks this item. Please remove the item before deleting it.'
        );
      }
