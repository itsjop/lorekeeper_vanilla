...
class CharacterCategoryService extends Service {
  ...
  public function updateCharacterCategory($category, $data) {
    ...
    try {
      if (
        CharacterCategory::where('name', $data['name'])->where('id', '!=', $category->id)->exists()
      ) {
        throw new \Exception('The name has already been taken.');
      }
      if (
        CharacterCategory::where('code', $data['code'])->where('id', '!=', $category->id)->exists()
      ) {
  ...
  public function deleteCharacterCategory($category) {
    ...
    try {
      ...
      if (Character::where('character_category_id', $category->id)->exists()) {
        throw new \Exception(
          'An character with this category exists. Please change its category first.'
        );
      }
