...
class GalleryService extends Service {
  ...
  public function deleteGallery($gallery) {
    ...
    try {
      ...
      if (
        GallerySubmission::where('gallery_id', $gallery->id)->exists() ||
        Gallery::where('parent_id', $gallery->id)->exists()
      ) {
        throw new \Exception(
          "A gallery or submissions in this gallery exist. Consider setting the gallery's submissions to closed instead."
        );
      }
