...
class LinkService extends Service {
    ...
    try {
      ...
      if (
        DB::table('user_aliases')
          ->where('site', $provider)
          ->where('alias', $result->nickname)
          ->exists()
      ) {
        throw new \Exception(
          'Cannot link the same account multiple times and/or to different site accounts.'
        );
      }
      ...
      UserUpdateLog::create([
        'user_id' => $user->id,
        'data' => json_encode(['alias' => $result->nickname, 'site' => $provider]),
        'type' => 'Alias Added'
      ]);
  ...
  public function makePrimary($aliasId, $user) {
    ...
    try {
      $alias = UserAlias::where('id', $aliasId)
        ->where('user_id', $user->id)
        ->where('is_primary_alias', 0)
        ->first();
      ...
      UserAlias::where('user_id', $user->id)
        ->where('is_primary_alias', 1)
        ->update(['is_primary_alias' => 0]);
      ...
      UserUpdateLog::create([
        'user_id' => $user->id,
        'data' => json_encode(['alias' => $alias->alias, 'site' => $alias->site]),
        'type' => 'Primary Alias Changed'
      ]);
