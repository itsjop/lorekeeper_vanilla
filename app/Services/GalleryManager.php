...
class GalleryManager extends Service {
  ...
  public function createSubmission($data, $currencyFormData, $user) {
    ...
    try {
      ...
      if (
        (isset($gallery->start_at) && $gallery->start_at->isFuture()) ||
        (isset($gallery->end_at) && $gallery->end_at->isPast())
      ) {
      ...
      if (isset($data['collaborator_id'])) {
        ...
        if (count($collaborators) != count($data['collaborator_id'])) {
          throw new \Exception(
            'One or more of the selected collaborators does not exist, or you have entered a duplicate.'
          );
        }
      } else {
        $collaborators = [];
      ...
      if (isset($data['participant_id'])) {
        ...
        if (count($participants) != count($data['participant_id'])) {
          throw new \Exception(
            'One or more of the selected participants does not exist, or you have entered a duplicate.'
          );
        }
      } else {
        $participants = [];
      ...
      if (isset($data['slug'])) {
        ...
        if (count($characters) != count($data['slug'])) {
          throw new \Exception(
            'One or more of the selected characters does not exist, or you have entered a duplicate.'
          );
        }
      } else {
        $characters = [];
      ...
      if (
        !$submission->collaborators->count() &&
        (!Settings::get('gallery_submissions_require_approval') ||
          (Settings::get('gallery_submissions_require_approval') &&
            $submission->gallery->votes_required == 0))
      ) {
    ...
    try {
      ...
      if ($submission->status == 'Pending') {
        ...
        if (isset($data['collaborator_id'])) {
          ...
          if (count($collaborators) != count($data['collaborator_id'])) {
            throw new \Exception(
              'One or more of the selected users does not exist, or you have entered a duplicate.'
            );
          }
        } else {
          $collaborators = [];
        ...
        if (isset($data['collaborator_id']) && $collaborators->count()) {
          ...
          foreach ($data['collaborator_id'] as $key => $collaborator) {
            GalleryCollaborator::create([
              ...
              'gallery_submission_id' => $submission->id,
              'data' => $data['collaborator_data'][$key],
              'has_approved' => isset($collaboratorApproval[$collaborator])
                ? $collaboratorApproval[$collaborator]
      ...
      // Check that the selected prompt exists and can be submitted to
      if (isset($data['prompt_id'])) {
        $prompt = $user->hasPower('manage_submissions')
          ? Prompt::find($data['prompt_id'])
          : Prompt::active()->find($data['prompt_id']);
        if (!$prompt) {
          throw new \Exception('Invalid prompt selected.');
        }
      }
      ...
      if (
        isset($submission->parsed_description) &&
        $submission->parsed_description &&
      ...
      if (
        $user->hasPower('manage_submissions') &&
        $user->id != $submission->user->id &&
        (isset($data['alert_user']) && $data['alert_user'])
      ) {
  ...
  public function castVote($action, $submission, $user) {
    ...
    try {
      ...
      $voteData = isset($submission->vote_data)
        ? collect(json_decode($submission->vote_data, true))
        : collect([]);
        ...
        foreach ($submission->characters as $character) {
          if ($character->user && $character->character->user->id != $submission->user->id) {
            Notifications::create('GALLERY_SUBMISSION_CHARACTER', $character->character->user, [
              ...
              'character' => isset($character->character->name)
  ...
  public function postValueSubmission($id, $data, $user) {
    ...
    try {
      ...
      if (!isset($data['ineligible'])) {
        ...
        $awardData =
          'Received reward for gallery submission (<a href="' .
          $submission->url .
          '">#' .
          $submission->id .
          '</a>)';
        ...
        if (isset($data['value']['submitted'])) {
          if (
            !$currencyManager->creditCurrency(
              ...
              $awardData,
              $currency,
              $data['value']['submitted'][$submission->user->id]
            )
          ) {
        ...
        if (isset($data['value']['collaborator'])) {
          foreach ($submission->collaborators as $collaborator) {
            if ($data['value']['collaborator'][$collaborator->user->id] > 0) {
              ...
              if (
                isset($data['value']['submitted']) &&
                $collaborator->user->id == $submission->user->id
              ) {
              ...
              if (
                !$currencyManager->creditCurrency(
                  ...
                  $awardData,
                  $currency,
                  $data['value']['collaborator'][$collaborator->user->id]
                )
              ) {
            ...
            if ($data['value']['participant'][$participant->user->id] > 0) {
              if (
                !$currencyManager->creditCurrency(
                  ...
                  $awardData,
                  $currency,
                  $data['value']['participant'][$participant->user->id]
                )
              ) {
  ...
  public function favoriteSubmission($submission, $user) {
    ...
    try {
      ...
      if (
        ...
        $submission->collaborators->where('user_id', $user->id)->first() != null
      ) {
