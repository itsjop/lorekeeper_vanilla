...
class CharacterManager extends Service {
  ...
  public function pullNumber($categoryId) {
    ...
    if (Config::get('lorekeeper.settings.character_pull_number') == 'all') {
      ...
      Config::get('lorekeeper.settings.character_pull_number') == 'category' &&
      $categoryId
    ) {
  ...
  public function createCharacter($data, $user, $isMyo = false) {
    ...
    try {
      ...
      if (
        ...
        !(isset($data['owner_url']) && $data['owner_url'])
      ) {
  ...
  private function handleCharacter($data, $isMyo = false) {
    try {
      if ($isMyo) {
        ...
        $data['slug'] = null;
        $data['species_id'] =
          isset($data['species_id']) && $data['species_id'] ? $data['species_id'] : null;
        $data['subtype_id'] =
  ...
  private function processImage($characterImage) {
    ...
    $image = Image::make($characterImage->imagePath . '/' . $characterImage->imageFileName)->trim(
      'transparent'
    );
    ...
    if (
      ...
      $canvas = Image::canvas(
        $image->width(),
        $image->height(),
        Config::get('lorekeeper.settings.masterlist_image_background')
      );
      $image = $canvas->insert($image, 'center');
    }
    ...
    if (Config::get('lorekeeper.settings.store_masterlist_fullsizes') == 1) {
      ...
      if (Config::get('lorekeeper.settings.masterlist_fullsizes_cap') != 0) {
        ...
        if ($imageWidth > $imageHeight) {
          // Landscape
          $image->resize(
            Config::get('lorekeeper.settings.masterlist_fullsizes_cap'),
            null,
            function ($constraint) {
      ...
      if (
        isset($characterImage->fullsize_hash)
          ? file_exists(
            public_path($characterImage->imageDirectory . '/' . $characterImage->fullsizeFileName)
          )
          : false
      ) {
    ...
    if (Config::get('lorekeeper.settings.masterlist_image_dimension') != 0) {
      ...
      if ($imageWidth > $imageHeight) {
        ...
        $image->resize(
          null,
          Config::get('lorekeeper.settings.masterlist_image_dimension'),
          function ($constraint) {
            $constraint->aspectRatio();
            $constraint->upsize();
          }
  ...
  private function cropThumbnail($points, $characterImage, $isMyo = false) {
    ...
    if (Config::get('lorekeeper.settings.watermark_masterlist_thumbnails') == 1 && !$isMyo) {
      ...
      $image->resize(
        Config::get('lorekeeper.settings.masterlist_thumbnails.width'),
        Config::get('lorekeeper.settings.masterlist_thumbnails.height')
      );
    }
  ...
  public function updateCharacterStats($data, $character, $user) {
    ...
    try {
      if (
        ...
        Character::where('slug', $data['slug'])->where('id', '!=', $character->id)->exists()
      ) {
      ...
      $characterData['is_giftable'] = isset($data['is_giftable']);
      $characterData['sale_value'] = isset($data['sale_value']) ? $data['sale_value'] : 0;
      $characterData['transferrable_at'] = isset($data['transferrable_at'])
        ? $data['transferrable_at']
        : null;
      if ($character->is_myo_slot) {
      ...
      if (!$character->is_myo_slot) {
        if ($characterData['character_category_id'] != $character->character_category_id) {
          ...
          $new['character_category'] = CharacterCategory::find(
            $characterData['character_category_id']
          )->displayName;
        }
  ...
  public function updateCharacterProfile($data, $character, $user, $isAdmin = false) {
    ...
    try {
      ...
      if (!$isAdmin) {
        ...
        if (
          isset($data['is_gift_art_allowed']) &&
          $character->is_gift_art_allowed != $data['is_gift_art_allowed']
        ) {
          $notifyGiftArt = true;
        }
        if (
          isset($data['is_gift_writing_allowed']) &&
          $character->is_gift_writing_allowed != $data['is_gift_writing_allowed']
        ) {
          $notifyGiftWriting = true;
        }

        $character->is_gift_art_allowed =
          isset($data['is_gift_art_allowed']) && $data['is_gift_art_allowed'] <= 2
            ? $data['is_gift_art_allowed']
            : 0;
        $character->is_gift_writing_allowed =
          isset($data['is_gift_writing_allowed']) && $data['is_gift_writing_allowed'] <= 2
            ? $data['is_gift_writing_allowed']
            : 0;
      ...
      if (
        !$character->is_myo_slot &&
        Config::get('lorekeeper.extensions.character_TH_profile_link')
      ) {
  ...
  public function createTransfer($data, $character, $user) {
    ...
    try {
      ...
      if ($character->transferrable_at && $character->transferrable_at->isFuture()) {
        throw new \Exception(
          'This character is still on transfer cooldown and cannot be transferred.'
        );
      }
        ...
        if ($update->status == 'Draft') {
          if (
            !$this->rejectRequest(
              'Cancelled by ' .
                $user->displayName .
                ' in order to transfer character to another user',
              $update,
              $user,
              true,
              false
  ...
  public function adminTransfer($data, $character, $user) {
    ...
    try {
      ...
      $this->moveCharacter(
        ...
        isset($data['cooldown']) ? $data['cooldown'] : -1
      );
      ...
      if ($sender) {
        Notifications::create('CHARACTER_SENT', $sender, [
          ...
          'sender_name' => $user->name,
          'sender_url' => $user->url,
          'recipient_name' => is_object($recipient)
            ? $recipient->name
            : prettyProfileName($recipient),
  ...
  public function processTransferQueue($data, $user) {
    ...
    try {
      ...
      if ($data['action'] == 'Approve') {
        ...
        $transfer->data = json_encode([
          'staff_id' => $user->id,
          'cooldown' => isset($data['cooldown'])
            ? $data['cooldown']
            : Settings::get('transfer_cooldown')
        ]);
  ...
  public function moveCharacter($character, $recipient, $data, $cooldown = -1, $logType = null) {
    ...
    if (
      Config::get('lorekeeper.settings.reset_character_profile_on_transfer') &&
      !$character->is_myo_slot
    ) {
  ...
  public function saveRequestImage($data, $request, $isAdmin = false) {
    ...
    try {
      ...
      if (
        !$isAdmin &&
        !isset($data['image']) &&
        !file_exists($request->imagePath . '/' . $request->imageFileName)
      ) {
      ...
      if (isset($data['currency_id'])) {
        foreach ($data['currency_id'] as $holderKey => $currencyIds) {
          ...
          foreach ($currencyIds as $key => $currencyId) {
            ...
            if (
              !$currencyManager->debitCurrency(
                $holder,
                null,
                null,
                null,
                $currency,
                $data['currency_quantity'][$holderKey][$key]
              )
            ) {
  ...
  public function approveRequest($data, $request, $user) {
    ...
    try {
      ...
      if (
        !isset($data['slug']) ||
        Character::where('slug', $data['slug'])->where('id', '!=', $request->character_id)->exists()
      ) {
        throw new \Exception('Please enter a unique character code.');
      }
      ...
      if (
        isset($requestData['character']['currencies']) &&
        $requestData['character']['currencies']
      ) {
        foreach ($requestData['character']['currencies'] as $currencyId => $quantity) {
          $currency = Currency::find($currencyId);
          if (
      ...
      $request
        ->designers()
        ->update(['character_type' => 'Character', 'character_image_id' => $image->id]);
      $request
        ->artists()
        ->update(['character_type' => 'Character', 'character_image_id' => $image->id]);
      ...
      $request
        ->rawFeatures()
        ->update(['character_image_id' => $image->id, 'character_type' => 'Character']);
      ...
      File::move(
        $request->imagePath . '/' . $request->imageFileName,
        $image->imagePath . '/' . $image->imageFileName
      );
  ...
  public function voteRequest($action, $request, $user) {
    ...
    try {
      ...
      $voteData = isset($request->vote_data)
        ? collect(json_decode($request->vote_data, true))
        : collect([]);
