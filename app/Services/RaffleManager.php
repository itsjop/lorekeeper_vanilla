...
class RaffleManager extends Service {
  ...
  public function addTicket($user, $raffle, $count = 1) {
    if (!$user) {
      ...
      DB::beginTransaction();
      $data =
        ['raffle_id' => $raffle->id, 'created_at' => Carbon::now()] +
        (is_string($user) ? ['alias' => $user] : ['user_id' => $user->id]);
      for ($i = 0; $i < $count; $i++) {
  ...
  private function rollWinners($raffle) {
    ...
    for ($i = 0; $i < $raffle->winner_count; $i++) {
      ...
      foreach ($ticketPool as $key => $ticket) {
        if (
          ($ticket->user_id != null && $ticket->user_id == $winner->user_id) ||
          ($ticket->user_id == null && $ticket->alias == $winner->alias)
        ) {
  ...
  private function afterRoll($winners, $raffleGroup, $raffle) {
    ...
    foreach ($raffles as $r) {
      $r->tickets()
        ->where(function ($query) use ($winners) {
