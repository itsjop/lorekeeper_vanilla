...
class SubmissionManager extends Service {
  ...
  public function createSubmission($data, $user, $isClaim = false) {
    ...
    try {
      ...
      if (isset($data['character_currency_id'])) {
        ...
        $data['character_rewardable_id'] = array_map(
          [$this, 'innerNull'],
  ...
  private function processRewards($data, $isCharacter, $isStaff = false) {
    if ($isCharacter) {
      ...
      if (
        isset($data['character_currency_id'][$data['character_id']]) &&
        isset($data['character_quantity'][$data['character_id']])
      ) {
        foreach ($data['character_currency_id'][$data['character_id']] as $key => $currency) {
          if ($data['character_quantity'][$data['character_id']][$key]) {
            addAsset(
              ...
              $data['character_quantity'][$data['character_id']][$key]
            );
          }
        }
      } elseif (
        isset($data['character_rewardable_type'][$data['character_id']]) &&
        isset($data['character_rewardable_id'][$data['character_id']]) &&
        isset($data['character_rewardable_quantity'][$data['character_id']])
      ) {
        $data['character_rewardable_id'] = array_map(
          [$this, 'innerNull'],
          $data['character_rewardable_id']
          ...
          switch ($data['character_rewardable_type'][$data['character_id']][$key]) {
            case 'Currency':
              if ($data['character_rewardable_quantity'][$data['character_id']][$key]) {
                addAsset(
                  $assets,
                  $data['currencies'][$reward],
                  $data['character_rewardable_quantity'][$data['character_id']][$key]
                );
              }
              break;
            case 'Item':
  ...
  public function rejectSubmission($data, $user) {
    ...
    try {
      ...
      Notifications::create(
        $submission->prompt_id ? 'SUBMISSION_REJECTED' : 'CLAIM_REJECTED',
        $submission->user,
        [
          'staff_url' => $user->url,
  ...
  public function approveSubmission($data, $user) {
    ...
    try {
        ...
        foreach ($stacks as $stackId => $quantity) {
          ...
          if (
            !$inventoryManager->debitStack(
              ...
              [
                'data' =>
                  'Item used in submission (<a href="' .
                  $submission->viewUrl .
                  '">#' .
                  $submission->id .
                  '</a>)'
        ...
        foreach ($addonData['currencies'] as $currencyId => $quantity) {
          ...
          if (
            !$currencyManager->createLog(
              ...
              $submission->prompt_id ? 'Prompt Approved' : 'Claim Approved',
              'Used in ' .
                ($submission->prompt_id ? 'prompt' : 'claim') .
                ' (<a href="' .
                $submission->viewUrl .
                '">#' .
                $submission->id .
      ...
      Notifications::create(
        $submission->prompt_id ? 'SUBMISSION_APPROVED' : 'CLAIM_APPROVED',
        $submission->user,
        [
          'staff_url' => $user->url,
          'staff_name' => $user->name,
