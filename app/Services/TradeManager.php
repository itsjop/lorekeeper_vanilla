...
class TradeManager extends Service {
  ...
  public function editTrade($data, $user) {
    ...
    try {
      ...
      if ($assetData = $this->handleTradeAssets($trade, $data, $user)) {
        ...
        $tradeData[$isSender ? 'sender' : 'recipient'] = getDataReadyAssets(
          $assetData[$isSender ? 'sender' : 'recipient']
        );
  ...
  private function handleTradeAssets($trade, $data, $user) {
    ...
    try {
      ...
      Character::where('trade_id', $trade->id)
      ...
      if (isset($data['character_id'])) {
        foreach ($data['character_id'] as $characterId) {
          ...
          if (CharacterTransfer::active()->where('character_id', $character->id)->exists()) {
            throw new \Exception(
              'One or more of the selected characters is already pending a character transfer.'
            );
          }
          ...
          if ($character->designUpdate()->active()->exists()) {
            throw new \Exception(
              'One or more of the selected characters has an active design update. Please wait for it to be processed, or delete it.'
            );
          }
          if ($character->transferrable_at && $character->transferrable_at->isFuture()) {
            throw new \Exception(
              'One or more of the selected characters is still on transfer cooldown and cannot be transferred.'
            );
          }
    ...
    try {
      ...
      if ($this->returnAttachments($trade)) {
        Notifications::create(
          'TRADE_CANCELED',
          $trade->sender_id == $user->id ? $trade->recipient : $trade->sender,
          [
            'sender_url' => $user->url,
            'sender_name' => $user->name,
            'trade_id' => $trade->id
  ...
  public function confirmTrade($data, $user) {
    ...
    try {
      ...
      if (!$trade->isConfirmable) {
        throw new \Exception(
          'Both parties are required to confirm their offers before the trade can be confirmed.'
        );
      }
      ...
      if ($trade->is_sender_trade_confirmed && $trade->is_recipient_trade_confirmed) {
        if (
          !(
            Settings::get('open_transfers_queue') &&
            (isset($trade->data['sender']['characters']) ||
              isset($trade->data['recipient']['characters']))
          )
        ) {
        ...
        Notifications::create(
          'TRADE_UPDATE',
          $user->id == $trade->sender_id ? $trade->recipient : $trade->sender,
          [
            'sender_url' => $user->url,
            'sender_name' => $user->name,
            'trade_id' => $trade->id
    ...
    try {
      ...
      if ($senderStacks) {
        foreach ($senderStacks as $stack) {
          ...
          $inventoryManager->moveStack(
            $trade->sender,
            $trade->recipient,
            'Trade',
            ['data' => 'Received in trade [<a href="' . $trade->url . '">#' . $trade->id . '</a>]'],
            $stack,
            $quantity
          );
      ...
      $senderCharacters = Character::where('user_id', $trade->sender_id)
        ->where('trade_id', $trade->id)
        ->get();
      $recipientCharacters = Character::where('user_id', $trade->recipient_id)
        ->where('trade_id', $trade->id)
        ->get();
      ...
      foreach ($senderCharacters as $character) {
        $characterManager->moveCharacter(
          $character,
          $trade->recipient,
          'Trade [<a href="' . $trade->url . '">#' . $trade->id . '</a>]',
          isset($cooldowns[$character->id]) ? $cooldowns[$character->id] : $defaultCooldown,
          'Transferred in trade'
        );
      }
          ...
          foreach ($tradeData[$type]['currencies'] as $currencyId => $quantity) {
            ...
            if (
              !$currencyManager->creditCurrency(
                ...
                $trade->{$recipientType},
                'Trade',
                'Received in trade [<a href="' . $trade->url . '">#' . $trade->id . ']',
                $currency,
                $quantity
              )
            ) {
