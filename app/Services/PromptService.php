...
class PromptService extends Service {
  ...
  public function updatePromptCategory($category, $data, $user) {
    ...
    try {
      ...
      if (
        PromptCategory::where('name', $data['name'])->where('id', '!=', $category->id)->exists()
      ) {
  ...
  public function deletePromptCategory($category) {
    ...
    try {
      ...
      if (Prompt::where('prompt_category_id', $category->id)->exists()) {
        throw new \Exception(
          'An prompt with this category exists. Please change its category first.'
        );
      }
  ...
  public function createPrompt($data, $user) {
    ...
    try {
      ...
      if (
        isset($data['prompt_category_id']) &&
        $data['prompt_category_id'] &&
        !PromptCategory::where('id', $data['prompt_category_id'])->exists()
      ) {
      ...
      $this->populateRewards(
        Arr::only($data, ['rewardable_type', 'rewardable_id', 'quantity']),
        $prompt
      );
  ...
  public function updatePrompt($prompt, $data, $user) {
    ...
    try {
      ...
      if (
        isset($data['prefix']) &&
        Prompt::where('prefix', $data['prefix'])->where('id', '!=', $prompt->id)->exists()
      ) {
  ...
  public function deletePrompt($prompt) {
    ...
    try {
      ...
      if (Submission::where('prompt_id', $prompt->id)->exists()) {
        throw new \Exception(
          'A submission under this prompt exists. Deleting the prompt will break the submission page - consider setting the prompt to be not active instead.'
        );
      }
