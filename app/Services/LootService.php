...
class LootService extends Service {
  ...
  public function deleteLootTable($table) {
    ...
    try {
      ...
      if (
        PromptReward::where('rewardable_type', 'LootTable')
          ->where('rewardable_id', $table->id)
          ->exists()
      ) {
        throw new \Exception(
          'A prompt uses this table to distribute rewards. Please remove it from the rewards list first.'
        );
      }
