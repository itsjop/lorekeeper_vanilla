...
class FeatureService extends Service {
  ...
  public function updateFeatureCategory($category, $data, $user) {
    ...
    try {
      ...
      if (
        FeatureCategory::where('name', $data['name'])->where('id', '!=', $category->id)->exists()
      ) {
  ...
  public function deleteFeatureCategory($category) {
    ...
    try {
      ...
      if (Feature::where('feature_category_id', $category->id)->exists()) {
        throw new \Exception(
          'A trait with this category exists. Please change its category first.'
        );
      }
  ...
  public function createFeature($data, $user) {
    ...
    try {
      ...
      if (
        isset($data['feature_category_id']) &&
        $data['feature_category_id'] &&
        !FeatureCategory::where('id', $data['feature_category_id'])->exists()
      ) {
      ...
      if (
        ...
        $data['species_id'] &&
        !Species::where('id', $data['species_id'])->exists()
      ) {
