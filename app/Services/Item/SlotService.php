...
class SlotService extends Service {
  ...
  public function getEditData() {
    return [
      ...
      'subtypes' =>
        ['0' => 'Select Subtype'] +
        Subtype::orderBy('sort', 'DESC')->pluck('name', 'id')->toArray(),
      'isMyo' => true
    ];
  }
  ...
  public function getTagData($tag) {
    ...
    $characterData['species_id'] =
      isset($tag->data['species_id']) && $tag->data['species_id'] ? $tag->data['species_id'] : null;
    $characterData['subtype_id'] =
      isset($tag->data['subtype_id']) && $tag->data['subtype_id'] ? $tag->data['subtype_id'] : null;
    $characterData['rarity_id'] =
      isset($tag->data['rarity_id']) && $tag->data['rarity_id'] ? $tag->data['rarity_id'] : null;
    $characterData['description'] =
      isset($tag->data['description']) && $tag->data['description']
  ...
  public function updateData($tag, $data) {
    ...
    $characterData['description'] =
      isset($data['description']) && $data['description'] ? $data['description'] : null;
    $characterData['parsed_description'] = parse($characterData['description']);
  ...
  public function act($stacks, $user, $data) {
    ...
    try {
      foreach ($stacks as $key => $stack) {
        ...
        if (
          (new InventoryManager())->debitStack(
            $stack->user,
            'Slot Used',
            ['data' => ''],
            $stack,
          ...
          for ($q = 0; $q < $data['quantities'][$key]; $q++) {
            ...
            $characterData['name'] = isset($characterData['name'])
              ? $characterData['name']
              : 'Slot';
            ...
            if ($character = $charService->createCharacter($characterData, $user, true)) {
              flash(
                '<a href="' . $character->url . '">MYO slot</a> created successfully.'
              )->success();
            } else {
