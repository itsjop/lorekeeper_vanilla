...
class ShopManager extends Service {
  ...
  public function buyStock($data, $user) {
    ...
    try {
      ...
      $shopStock = ShopStock::where('id', $data['stock_id'])
      ...
      if ($shopStock->purchase_limit && $this->checkPurchaseLimitReached($shopStock, $user)) {
        throw new \Exception(
          'You have already purchased the maximum amount of this item you can buy.'
        );
      }
      ...
      if ($data['bank'] == 'character') {
        ...
        if (
          !(new CurrencyManager())->debitCurrency(
            $character,
            null,
            'Shop Purchase',
            'Purchased ' . $shopStock->item->name . ' from ' . $shop->name,
            $shopStock->currency,
            $total_cost
          )
        ) {
      ...
      if (
        !(new InventoryManager())->creditItem(
          null,
          $user,
          'Shop Purchase',
          [
  ...
  public function getStockPurchaseLimit($shopStock, $user) {
    ...
    if ($shopStock->purchase_limit > 0) {
      $user_purchase_limit =
        $shopStock->purchase_limit - $this->checkUserPurchases($shopStock, $user);
