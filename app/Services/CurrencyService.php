...
class CurrencyService extends Service {
  ...
  public function createCurrency($data, $user) {
    ...
    try {
      ...
      if (!isset($data['is_user_owned']) && !isset($data['is_character_owned'])) {
        throw new \Exception(
          'Please choose if this currency is attached to users and/or characters.'
        );
      }
  ...
  public function updateCurrency($currency, $data, $user) {
    ...
    try {
      ...
      if (
        isset($data['abbreviation']) &&
        Currency::where('abbreviation', $data['abbreviation'])
  ...
  public function deleteCurrency($currency) {
    ...
    try {
      if (
        DB::table('loots')
          ->where('rewardable_type', 'Currency')
          ->where('rewardable_id', $currency->id)
          ->exists()
      ) {
        throw new \Exception(
          'A loot table currently distributes this currency as a potential reward. Please remove the currency before deleting it.'
        );
      }
      if (
        ...
        throw new \Exception(
          'A prompt currently distributes this currency as a reward. Please remove the currency before deleting it.'
        );
      }
      if (DB::table('shop_stock')->where('currency_id', $currency->id)->exists()) {
        throw new \Exception(
          'A shop currently requires this currency to purchase an item. Please change the currency before deleting it.'
        );
      }
  ...
  public function sortCurrency($data, $type) {
    ...
    try {
      ...
      foreach ($sort as $key => $s) {
        Currency::where('id', $s)->update([
          $type == 'user' ? 'sort_user' : 'sort_character' => $key
        ]);
      }
