...
class BrowseController extends Controller {
  ...
  public function getUsers(Request $request) {
    ...
    return view('browse.users', [
      ...
      'ranks' =>
        [0 => 'Any Rank'] + Rank::orderBy('ranks.sort', 'DESC')->pluck('name', 'id')->toArray(),
  ...
  public function getCharacters(Request $request) {
    ...
    if ($request->get('name')) {
      $query->where(function ($query) use ($request) {
        $query
          ...
          ->orWhere('characters.slug', 'LIKE', '%' . $request->get('name') . '%');
      });
    }
    ...
    // Search only main images
    if (
      !$request->get('search_images') &&
      !$request->get('transformation_id') &&
    ...
    return view('browse.masterlist', [
      ...
      'categories' =>
        [0 => 'Any Category'] +
        CharacterCategory::whereNotIn('id', $subCategories)
          ->orderBy('character_categories.sort', 'DESC')
          ->pluck('name', 'id')
          ->toArray(),
      'specieses' =>
        [0 => 'Any Species'] +
        Species::whereNotIn('id', $subSpecies)
          ->orderBy('specieses.sort', 'DESC')
          ->pluck('name', 'id')
          ->toArray(),
      'subtypes' =>
        [0 => 'Any Subtype'] +
        Subtype::orderBy('subtypes.sort', 'DESC')->pluck('name', 'id')->toArray(),
      'rarities' =>
        [0 => 'Any Rarity'] +
        Rarity::orderBy('rarities.sort', 'DESC')->pluck('name', 'id')->toArray(),
      'features' => Feature::orderBy('features.name')->pluck('name', 'id')->toArray(),
      'sublists' => Sublist::orderBy('sort', 'DESC')->get(),
      'userOptions' => User::query()->orderBy('name')->pluck('name', 'id')->toArray(),
      'transformations' =>
        [0 => 'Any ' . ucfirst(__('transformations.transformation'))] +
        Transformation::orderBy('sort', 'DESC')->pluck('name', 'id')->toArray()
    ]);
  ...
  public function getMyos(Request $request) {
    ...
    return view('browse.myo_masterlist', [
      ...
      'specieses' =>
        [0 => 'Any Species'] +
        Species::orderBy('specieses.sort', 'DESC')->pluck('name', 'id')->toArray(),
      'rarities' =>
        [0 => 'Any Rarity'] +
        Rarity::orderBy('rarities.sort', 'DESC')->pluck('name', 'id')->toArray(),
      'features' => Feature::orderBy('features.name')->pluck('name', 'id')->toArray(),
  ...
  public function getSublist(Request $request, $key) {
    $query = Character::with('user.rank')
      ...
      ->with('image.species')
      ->myo(0);
    $imageQuery = CharacterImage::with('features')
      ->with('rarity')
      ->with('species')
      ->with('features');
    ...
    if ($request->get('feature_id')) {
      ...
      $subCategory = CharacterCategory::where('masterlist_sub_id', $sublist->id)
        ->orderBy('character_categories.sort', 'DESC')
        ->pluck('name', 'id')
        ->toArray();
      if (!$subCategory) {
        $subCategory = CharacterCategory::orderBy('character_categories.sort', 'DESC')
          ->pluck('name', 'id')
          ->toArray();
      }
      $subSpecies = Species::where('masterlist_sub_id', $sublist->id)
        ->orderBy('specieses.sort', 'DESC')
        ->pluck('name', 'id')
        ->toArray();
      if (!$subSpecies) {
