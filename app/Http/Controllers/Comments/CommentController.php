    ...
    switch ($model_type) {
      ...
      case 'App\Models\Report\Report':
        ...
        if ($recipients == $sender) {
          $recipient = isset($report->staff_id)
            ? $report->staff
            : User::find(Settings::get('admin_user'));
        } else {
      ...
      case 'App\Models\Gallery\GallerySubmission':
        ...
        $post =
          $type != 'User-User'
            ? 'your gallery submission\'s staff comments'
            : 'your gallery submission';
        $link =
          $type != 'User-User'
            ? $submission->queueUrl . '/#comment-' . $comment->getKey()
            : $submission->url . '/#comment-' . $comment->getKey();
        break;
    }
