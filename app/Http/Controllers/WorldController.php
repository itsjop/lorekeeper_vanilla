...
class WorldController extends Controller {
  ...
  public function getSpeciesFeatures($id) {
    ...
    $features = count($categories)
      ? $species
        ->features()
        ->orderByRaw(
          'FIELD(feature_category_id,' . implode(',', $categories->pluck('id')->toArray()) . ')'
        )
  ...
  public function getItems(Request $request) {
    ...
    return view('world.items', [
      ...
      'categories' =>
        ['none' => 'Any Category'] +
        ItemCategory::orderBy('sort', 'DESC')->pluck('name', 'id')->toArray(),
      'shops' => Shop::orderBy('sort', 'DESC')->get(),
      'artists' =>
        ['none' => 'Any Artist'] +
        User::whereIn('id', Item::whereNotNull('artist_id')->pluck('artist_id')->toArray())
          ->pluck('name', 'id')
          ->toArray()
    ]);
  }
  ...
  public function getItem($id) {
    ...
    return view('world.item_page', [
      ...
      'shops' => Shop::whereIn(
        'id',
        ShopStock::where('item_id', $item->id)->pluck('shop_id')->unique()->toArray()
      )
        ->orderBy('sort', 'DESC')
        ->get()
    ]);
  ...
  public function getPrompts(Request $request) {
    ...
    return view('world.prompts', [
      ...
      'categories' =>
        ['none' => 'Any Category'] +
        PromptCategory::orderBy('sort', 'DESC')->pluck('name', 'id')->toArray()
    ]);
  }
}
