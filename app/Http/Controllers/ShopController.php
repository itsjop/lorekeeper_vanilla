...
class ShopController extends Controller {
  ...
  public function getShop($id) {
    ...
    $items = count($categories)
      ...
      : $shop->displayStock()->orderBy('name')->get()->groupBy('item_category_id');
    return view('shops.shop', [
      ...
      'currencies' => Currency::whereIn(
        'id',
        ShopStock::where('shop_id', $shop->id)->pluck('currency_id')->toArray()
      )
        ->get()
        ->keyBy('id')
    ]);
  ...
  public function getShopStock(ShopManager $service, $id, $stockId) {
    ...
    if ($user) {
      ...
      $userOwned = UserItem::where('user_id', $user->id)
  ...
  public function postBuy(Request $request, ShopManager $service) {
    ...
    if (
      $service->buyStock(
        $request->only(['stock_id', 'shop_id', 'slug', 'bank', 'quantity']),
        Auth::user()
      )
    ) {
