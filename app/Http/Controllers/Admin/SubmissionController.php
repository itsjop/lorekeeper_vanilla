...
class SubmissionController extends Controller {
  ...
  public function getSubmission($id) {
    ...
    $inventory = isset($submission->data['user'])
      ? parseAssetData($submission->data['user'])
      : null;
    if (!$submission) {
      abort(404);
    }
    return view(
      ...
      [
        ...
        'page' => 'submission',
        'expanded_rewards' => Config::get(
          'lorekeeper.extensions.character_reward_expansion.expanded'
        )
      ] +
        ($submission->status == 'Pending'
          ? [
            ...
            'raffles' => Raffle::where('rolled_at', null)
              ...
              ->orderBy('name')
              ->pluck('name', 'id'),
            'count' => Submission::where('prompt_id', $submission->prompt_id)
        ...
        ($submission->status == 'Pending'
          ? [
            ...
            'raffles' => Raffle::where('rolled_at', null)
              ...
              ->orderBy('name')
              ->pluck('name', 'id'),
            'count' => Submission::where('prompt_id', $id)
              ->where('status', 'Approved')
              ->where('user_id', $submission->user_id)
              ->count(),
            'rewardsData' => isset($submission->data['rewards'])
  ...
  public function postSubmission(Request $request, SubmissionManager $service, $id, $action) {
    ...
    if (
      $action == 'reject' &&
      $service->rejectSubmission($request->only(['staff_comments']) + ['id' => $id], Auth::user())
    ) {
      flash('Submission rejected successfully.')->success();
    } elseif (
      $action == 'approve' &&
      $service->approveSubmission($data + ['id' => $id], Auth::user())
    ) {
