...
class CharacterController extends Controller {
  ...
  public function getEditCharacterStats($slug) {
    ...
    return view('character.admin._edit_stats_modal', [
      ...
      'userOptions' => User::query()->orderBy('name')->pluck('name', 'id')->toArray(),
      'number' => format_masterlist_number(
        $this->character->number,
        Config::get('lorekeeper.settings.character_number_digits')
      ),
      'isMyo' => false
    ]);
  }
  ...
  public function postTransfer(Request $request, CharacterManager $service, $slug) {
    ...
    if (
      $service->adminTransfer(
        $request->only(['recipient_id', 'recipient_url', 'cooldown', 'reason']),
        $this->character,
        Auth::user()
      )
    ) {
  ...
  public function getTransferQueue($type) {
    ...
    return view('admin.masterlist.character_transfers', [
      ...
      'transferCount' => $openTransfersQueue
        ? CharacterTransfer::active()->where('is_approved', 0)->count()
        : 0,
  ...
  public function postTransferQueue(Request $request, CharacterManager $service, $id) {
    ...
    if (
      $service->processTransferQueue(
        $request->only(['action', 'cooldown', 'reason']) + ['transfer_id' => $id],
        Auth::user()
      )
    ) {
  ...
  public function postTradeQueue(Request $request, TradeManager $service, $id) {
    ...
    if (
      $action == 'approve' &&
      $service->approveTrade($request->only(['action', 'cooldowns']) + ['id' => $id], Auth::user())
    ) {
      flash('Trade approved.')->success();
    } elseif (
      $action == 'reject' &&
      $service->rejectTrade($request->only(['action', 'reason']) + ['id' => $id], Auth::user())
    ) {
