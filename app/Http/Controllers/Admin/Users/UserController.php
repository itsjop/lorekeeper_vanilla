...
class UserController extends Controller {
    ...
    if ($request->get('name')) {
      $query->where(function ($query) use ($request) {
        $query
          ...
          ->orWhere('users.alias', 'LIKE', '%' . $request->get('name') . '%');
      });
    }
  ...
  public function postUserBasicInfo(Request $request, $name) {
    ...
    if (!$user) {
      ...
      flash(
        'You cannot edit the information of a user that has a higher rank than yourself.'
      )->error();
    } else {
      $request->validate([
  ...
  public function postUserAlias(Request $request, $name, $id) {
    ...
    $logData = [
      'old_alias' => $user ? $alias->alias : null,
      'old_site' => $user ? $alias->site : null
    ];
      ...
      if ($isPrimary) {
        if (!$user->aliases->count()) {
          ...
          foreach ($user->aliases as $alias) {
            if (
              Config::get('lorekeeper.sites.' . $alias->site . '.auth') &&
              Config::get('lorekeeper.sites.' . $alias->site . '.primary_alias') &&
              $alias->is_visible
            ) {
  ...
  public function postUserAccount(Request $request, $name) {
    ...
    if (!$user) {
      ...
      UserUpdateLog::create([
        'staff_id' => Auth::user()->id,
        'user_id' => $user->id,
        'data' => json_encode(['is_fto' => $request->get('is_fto') ? 'Yes' : 'No']),
        'type' => 'FTO Status Change'
      ]);
  ...
  public function postUserBirthday(Request $request, $name) {
    ...
    $logData = [
      'old_date' => $user->birthday
        ? $user->birthday->isoFormat('DD-MM-YYYY')
        : Carbon::now()->isoFormat('DD-MM-YYYY')
    ] + ['new_date' => $date];
  ...
  public function postBan(Request $request, UserService $service, $name) {
    ...
    if (!$user) {
      ...
      flash(
        $wasBanned ? 'User ban reason edited successfully.' : 'User banned successfully.'
      )->success();
    } else {
      foreach ($service->errors()->getMessages()['error'] as $error) {
        flash($error)->error();
