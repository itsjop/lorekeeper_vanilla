...
class GalleryController extends Controller {
  ...
  public function getGallery($id, Request $request) {
    ...
    $query = GallerySubmission::where('gallery_id', $gallery->id)
    ...
    return view('galleries.gallery', [
      ...
      'prompts' =>
        ...
        Prompt::whereIn(
          'id',
          GallerySubmission::where('gallery_id', $gallery->id)
            ->visible(Auth::check() ? Auth::user() : null)
            ->accepted()
          ...
          ->pluck('name', 'id')
          ->toArray(),
      'childSubmissions' => GallerySubmission::whereIn(
        'gallery_id',
  ...
  public function getSubmission($id) {
    ...
    return view('galleries.submission', [
      ...
      'commentCount' => Comment::where('commentable_type', 'App\Models\Gallery\GallerySubmission')
        ->where('commentable_id', $submission->id)
  ...
  public function getNewGallerySubmission(Request $request, $id) {
    ...
    return view(
      'galleries.create_edit_submission',
      [
        'closed' => $closed
      ] +
        ($closed
  ...
  public function getEditGallerySubmission($id) {
    ...
    $prompts =
      Auth::user()->hasPower('manage_submissions') && $submission->status == 'Pending'
        ? Prompt::query()
        : Prompt::active();
  ...
  public function postCreateEditGallerySubmission(
    Request $request,
    GalleryManager $service,
    $id = null
  ) {
    $id
      ? $request->validate(GallerySubmission::$updateRules)
      : $request->validate(GallerySubmission::$createRules);
    $data = $request->only([
      'image',
      'text',
      'title',
    ...
    if (!$id && Settings::get('gallery_submissions_reward_currency')) {
      $currencyFormData = $request->only(
        collect(Config::get('lorekeeper.group_currency_form'))->keys()->toArray()
      );
    } else {
    ...
    if ($id && $service->updateSubmission(GallerySubmission::find($id), $data, Auth::user())) {
      ...
      !$id &&
      ($gallery = $service->createSubmission($data, $currencyFormData, Auth::user()))
    ) {
