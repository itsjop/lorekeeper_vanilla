...
class InventoryController extends Controller {
  ...
  public function getStack(Request $request, $id) {
    ...
    $readOnly =
      ...
      (Auth::check() &&
      $first_instance &&
      ($first_instance->user_id == Auth::user()->id || Auth::user()->hasPower('edit_inventories'))
        ? 0
        : 1);
    $stack = UserItem::where([
      ['user_id', $first_instance->user_id],
      ['item_id', $first_instance->item_id],
      ['count', '>', 0]
    ])->get();
    ...
    return view('home._inventory_stack', [
      ...
      'userOptions' =>
        ['' => 'Select User'] +
        User::visible()
          ->where('id', '!=', $first_instance ? $first_instance->user_id : 0)
          ->orderBy('name')
          ->get()
          ->pluck('verified_name', 'id')
          ->toArray(),
      'readOnly' => $readOnly,
      'characterOptions' => Character::visible()
        ->myo(0)
  ...
  public function getCharacterStack(Request $request, $id) {
    ...
    $stack = CharacterItem::where([
      ['character_id', $first_instance->character_id],
      ['item_id', $first_instance->item_id],
      ['count', '>', 0]
    ])->get();
    $item = Item::where('id', $first_instance->item_id)->first();
    ...
    isset($stack->first()->character->user_id)
      ? ($ownerId = $stack->first()->character->user_id)
      : null;

    $hasPower = Auth::check() ? Auth::user()->hasPower('edit_inventories') : false;
    $readOnly =
      $request->get('read_only') ?:
      (Auth::check() && $first_instance && (isset($ownerId) == true || $hasPower == true)
        ? 0
        : 1);
  ...
  private function postTransferToCharacter(Request $request, InventoryManager $service) {
    if (
      $service->transferCharacterStack(
        Auth::user(),
        Character::visible()->where('id', $request->get('character_id'))->first(),
        UserItem::find($request->get('ids')),
        $request->get('quantities')
      )
    ) {
  ...
  private function postAct(Request $request) {
    ...
    $tag = $request->get('tag');
    $service = $stacks->first()->item->hasTag($tag)
      ? $stacks->first()->item->tag($tag)->service
      : null;
  ...
  public function getAccountSearch(Request $request) {
    ...
    if ($item) {
      ...
      $characterItems = CharacterItem::whereIn('character_id', $characters->pluck('id')->toArray())
        ->where('item_id', $item->id)
        ->where('count', '>', 0)
        ->get();

      // Gather hold locations
      $designUpdates = CharacterDesignUpdate::where('user_id', $user->id)
        ->whereNotNull('data')
        ->get();
