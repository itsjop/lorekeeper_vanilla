...
function add_help($text) {
  return '<i class="fas fa-question-circle help-icon" data-toggle="tooltip" title="' .
    $text .
    '"></i>';
}
...
function format_date($timestamp, $showTime = true) {
  return $timestamp->format('j F Y' . ($showTime ? ', H:i:s' : '')) .
    ($showTime
      ? ' <abbr data-toggle="tooltip" title="UTC' .
        $timestamp->timezone->toOffsetName() .
        '">' .
        strtoupper($timestamp->timezone->getAbbreviatedName($timestamp->isDST())) .
        '</abbr>'
      : '');
}

function pretty_date($timestamp, $showTime = true) {
  return '<abbr data-toggle="tooltip" title="' .
    $timestamp->format('F j Y' . ($showTime ? ', H:i:s' : '')) .
    ' ' .
    strtoupper($timestamp->timezone->getAbbreviatedName($timestamp->isDST())) .
    '">' .
...
function parse($text, &$pings = null) {
  ...
  if ($def = $config->maybeGetRawHTMLDefinition()) {
    $def->addElement('include', 'Block', 'Empty', 'Common', [
      'file*' => 'URI',
      'height' => 'Text',
      'width' => 'Text'
    ]);
      ...
      if ($submission) {
        ...
        $text = preg_replace(
          ...
          '<a href="' .
            ...
            ' by ' .
            nl2br(htmlentities($submission->creditsPlain)) .
            (isset($submission->content_warning)
              ? '<br/><strong>Content Warning:</strong> ' .
                nl2br(htmlentities($submission->content_warning))
              : '') .
            '">' .
            view('widgets._gallery_thumb', ['submission' => $submission]) .
            '</a>',
          $text
        );
      }
...
function checkAlias($url, $failOnError = true) {
  if ($url) {
    ...
    if ((!isset($matches[0]) || $matches[0] == []) && $failOnError) {
      throw new \Exception(
        'This URL is from an invalid site. Please provide a URL for a user profile from a site used for authentication.'
      );
    }

    // and 2. if it contains an alias associated with a user on-site.
    if (isset($matches[0]) && $matches[0] != [] && isset($matches[0][1])) {
      $alias = App\Models\User\UserAlias::where('site', $urlSite)
        ->where('alias', $matches[0][0])
        ->first();
      if ($alias) {
...
function prettyProfileLink($url) {
  ...
  if (isset($name) && isset($site) && isset($link)) {
    return '<a href="https://' .
      ...
      $name .
      '@' .
      (Config::get('lorekeeper.sites.' . $site . '.display_name') != null
        ? Config::get('lorekeeper.sites.' . $site . '.display_name')
        : $site) .
      '</a>';
  } else {
