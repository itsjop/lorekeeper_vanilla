...
function fillUserAssets($assets, $sender, $recipient, $logType, $data) {
  ...
  foreach ($assets as $key => $contents) {
    if ($key == 'items' && count($contents)) {
      ...
      foreach ($contents as $asset) {
        if (
          !$service->moveCharacter($asset['asset'], $recipient, $data, $asset['quantity'], $logType)
        ) {
          return false;
        }
      }
...
function fillCharacterAssets($assets, $sender, $recipient, $logType, $data, $submitter = null) {
  if (
    !Config::get('lorekeeper.extensions.character_reward_expansion.default_recipient') &&
    $recipient->user
  ) {
    ...
    if ($key == 'currencies' && count($contents)) {
      ...
      foreach ($contents as $asset) {
        if (
          !$service->creditCurrency(
            $sender,
            $asset['asset']->is_character_owned ? $recipient : $item_recipient,
            $logType,
            $data['data'],
            $asset['asset'],
            $asset['quantity']
      ...
      foreach ($contents as $asset) {
        if (
          !$service->creditItem(
            $sender,
            $asset['asset']->category && $asset['asset']->category->is_character_owned
              ? $recipient
              : $item_recipient,
            $logType,
            $data,
