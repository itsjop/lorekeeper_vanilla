...
class SetupAdminUser extends Command {
  ...
  public function handle() {
    ...
    if (!$user) {
      $this->line(
        'Setting up admin account. This account will have access to all site data, please make sure to keep the email and password secret!'
      );
      ...
      if (App::environment('local')) {
        if ($this->confirm('Are you on a local/testing instance and not a live site?')) {
          if (
            $this->confirm(
              'Would you like to mark your email address as verified and enter an alias now?'
            )
          ) {
            $verifiedAt = Carbon::now();
            $this->line(
              'Provide an alias. By default, this will be entered as a deviantArt account name, but (assuming a local/testing environment) you may use any username.'
            );
            $alias = $this->ask('What alias would you like to use?');
          }
        } else {
          $this->info(
            'Please adjust your APP_ENV to Production and APP_DEBUG to false in your .env file before continuing set-up!'
          );
          return;
        }
      }
      ...
      if ($confirm) {
        ...
        $this->line(
          'Admin account created. You can now log in with the registered email and password.'
        );
        $this->line(
          'If necessary, you can run this command again to change the email address and password of the admin account.'
        );
        return;
      }
    } else {
