  ...
  public function handle() {
    ...
    foreach ($extensions as $data) {
      ...
      if (!$extension->exists()) {
        ...
        $this->info(
          ucfirst($data['key']) .
            ' version mismatch. Old version: ' .
            $extension->first()->version .
            ' / New version: ' .
            $data['version']
        );
        $confirm = $this->confirm(
          'Do you want to update the listed version of ' .
            $data['key'] .
            ' to ' .
            $data['version'] .
            '? This will not affect any other files.'
        );
        if ($confirm) {
          DB::table('site_extensions')
            ->where('key', $data['key'])
            ->update([
              'key' => $data['key'],
              'wiki_key' => $data['wiki_key'],
