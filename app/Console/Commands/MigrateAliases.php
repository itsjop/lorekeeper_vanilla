...
class MigrateAliases extends Command {
  ...
  public function handle() {
    ...
    if (Schema::hasColumn('users', 'alias')) {
      ...
      if ($aliasUsers->count()) {
        foreach ($aliasUsers as $user) {
          if (
            !DB::table('user_aliases')
              ->where('user_id', $user->id)
              ->where('site', 'deviantart')
    ...
    if (Schema::hasColumn('characters', 'owner_alias')) {
      ...
      if ($aliasCharacters->count()) {
        foreach ($aliasCharacters as $character) {
          ...
          $userAlias = UserAlias::where('site', 'deviantart')
            ->where('alias', $character->owner_alias)
            ->first();
          if ($userAlias) {
            ...
            $character->update([
              'owner_alias' => null,
              'owner_url' => 'https://deviantart.com/' . $alias
            ]);
          }
        }
    ...
    if (
      Schema::hasColumn('character_log', 'recipient_alias') ||
      Schema::hasColumn('character_log', 'sender_alias')
    ) {
      ...
      if ($aliasCharacterLogs->count()) {
        foreach ($aliasCharacterLogs as $characterLog) {
          $userAlias = UserAlias::where('site', 'deviantart')
            ->where('alias', $characterLog->recipient_alias)
            ->first();
          if ($userAlias) {
            $characterLog->update([
              'recipient_alias' => null,
              'recipient_id' => $userAlias->user_id
            ]);
          } elseif (!$userAlias) {
            $alias = $characterLog->recipient_alias;
            $characterLog->update([
              'recipient_alias' => null,
              'recipient_url' => 'https://deviantart.com/' . $alias
            ]);
          }
        }
        ...
        foreach ($aliasCharacterLogsSender as $characterLog) {
          $userAlias = UserAlias::where('site', 'deviantart')
            ->where('alias', $characterLog->sender_alias)
            ->first();
          if ($userAlias) {
            ...
            $characterLog->update([
              'sender_alias' => null,
              'sender_url' => 'https://deviantart.com/' . $alias
            ]);
          }
        }
    ...
    if (
      Schema::hasColumn('user_character_log', 'recipient_alias') ||
      Schema::hasColumn('user_character_log', 'sender_alias')
    ) {
      // Get character logs with a set recipient alias
    ...
    if (Schema::hasColumn('items', 'artist_alias')) {
      ...
      if ($aliasItemArtists->count()) {
        foreach ($aliasItemArtists as $itemArtist) {
          $userAlias = UserAlias::where('site', 'deviantart')
            ->where('alias', $itemArtist->artist_alias)
            ->first();
          if ($userAlias) {
            ...
            $itemArtist->update([
              'artist_alias' => null,
              'artist_url' => 'https://deviantart.com/' . $alias
            ]);
          }
        }
