...
class Trade extends Model {
  ...
  protected $fillable = [
    ...
    'status',
    'is_sender_confirmed',
    'is_recipient_confirmed',
    'is_sender_trade_confirmed',
    'is_recipient_trade_confirmed',
    'is_approved',
    'reason',
    'data'
  ];
  ...
  public function getCharacterData() {
    return Character::with('user')
      ->whereIn(
        'id',
        array_merge($this->getCharacters($this->sender), $this->getCharacters($this->recipient))
      )
      ->get();
  }
  ...
  public function getInventory($user) {
    ...
    $inventory =
      $this->data && isset($this->data[$type]) && isset($this->data[$type]['user_items'])
        ? $this->data[$type]['user_items']
        : [];
    return $inventory;
  }
  ...
  public function getCharacters($user) {
    $type = $this->sender_id == $user->id ? 'sender' : 'recipient';
    $characters =
      $this->data && isset($this->data[$type]) && isset($this->data[$type]['characters'])
        ? $this->data[$type]['characters']
        : [];
    if ($characters) {
