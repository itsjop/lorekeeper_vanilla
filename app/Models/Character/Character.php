...
class Character extends Model {
  ...
  public function items() {
    return $this->belongsToMany('App\Models\Item\Item', 'character_items')
      ...
      ->whereNull('character_items.deleted_at');
  }
  ...
  public function getCurrencies($displayedOnly = false) {
    ...
    $owned = CharacterCurrency::where('character_id', $this->id)
      ->pluck('quantity', 'currency_id')
      ->toArray();
  ...
  public function getCurrencySelect() {
    return CharacterCurrency::where('character_id', $this->id)
      ->leftJoin('currencies', 'character_currencies.currency_id', '=', 'currencies.id')
      ->orderBy('currencies.sort_character', 'DESC')
      ->get()
      ->pluck('name_with_quantity', 'currency_id')
      ->toArray();
  }
  ...
  public function getSubmissions() {
    return Submission::with('user.rank')
      ...
      ->whereIn(
        'id',
        SubmissionCharacter::where('character_id', $this->id)->pluck('submission_id')->toArray()
      )
      ->paginate(30);
  ...
  public function notifyBookmarkers($type) {
    ...
    if ($this->is_visible) {
      ...
      $bookmarkers = CharacterBookmark::where('character_id', $this->id)->where(
        'user_id',
        '!=',
        $this->user_id
      );
