...
class GallerySubmission extends Model {
  ...
  public function scopeCollaboratorApproved($query) {
    return $query->whereNotIn(
      'id',
      GalleryCollaborator::where('has_approved', 0)->pluck('gallery_submission_id')->toArray()
    );
  }
  ...
  public function scopeUserSubmissions($query, $user) {
    return $query
      ...
      ->orWhereIn(
        'id',
        GalleryCollaborator::where('user_id', $user->id)
          ->where('type', 'Collab')
  ...
  public function getPrefixAttribute() {
    $currencyName = Currency::find(Settings::get('group_currency'))->abbreviation
      ? Currency::find(Settings::get('group_currency'))->abbreviation
      : Currency::find(Settings::get('group_currency'))->name;
  ...
  public function getExcerptAttribute() {
    if (!isset($this->parsed_text)) {
      ...
      return strip_tags(substr($this->parsed_text, 0, 500)) .
        (strlen($this->parsed_text) > 500 ? '...' : '');
    }
  }
}
