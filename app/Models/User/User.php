...
class User extends Authenticatable implements MustVerifyEmail {
  ...
  public function items() {
    return $this->belongsToMany('App\Models\Item\Item', 'user_items')
      ...
      ->whereNull('user_items.deleted_at');
  }
  ...
  public function gallerySubmissions() {
    return $this->hasMany('App\Models\Gallery\GallerySubmission')
      ...
      ->orWhereIn(
        'id',
        GalleryCollaborator::where('user_id', $this->id)
          ->where('type', 'Collab')
  ...
  public function getDisplayNameAttribute() {
    return ($this->is_banned ? '<strike>' : '') .
      ...
      $this->url .
      '" class="display-user" ' .
      ($this->rank->color ? 'style="color: #' . $this->rank->color . ';"' : '') .
      '>' .
      $this->name .
      '</a>' .
  ...
  public function getCommentDisplayNameAttribute() {
    return '<small><a href="' .
      ...
      ($this->rank->color
        ? 'style="background-color: #' . $this->rank->color . '!important;color:#000!important;"'
        : '') .
      '><i class="' .
  ...
  public function getCurrencySelect($isTransferrable = false) {
    $query = UserCurrency::query()
      ...
      ->orderBy('currencies.sort_user', 'DESC');
    if ($isTransferrable) {
  ...
  public function updateCharacters() {
    ...
    foreach ($this->aliases as $alias) {
      ...
      foreach ($urlCharacters as $key => $character) {
        preg_match_all(
          Config::get('lorekeeper.sites.' . $alias->site . '.regex'),
          $character,
          $matches[$key]
        );
      }
