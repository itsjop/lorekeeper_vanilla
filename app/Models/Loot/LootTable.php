...
class LootTable extends Model {
    ...
    for ($i = 0; $i < $quantity; $i++) {
      ...
      if ($result) {
        ...
        if ($result->rewardable_type == 'LootTable') {
          ...
          $result->rewardable_type == 'ItemCategory' ||
          $result->rewardable_type == 'ItemCategoryRarity'
        ) {
          $rewards = mergeAssetsArrays(
            $rewards,
            $this->rollCategory(
              $result->rewardable_id,
              $result->quantity,
              isset($result->data['criteria']) ? $result->data['criteria'] : null,
              isset($result->data['rarity']) ? $result->data['rarity'] : null
            )
          );
  ...
  public function rollCategory($id, $quantity = 1, $criteria = null, $rarity = null) {
    ...
    if (isset($criteria) && $criteria && isset($rarity) && $rarity) {
      if (
        Config::get('lorekeeper.extensions.item_entry_expansion.loot_tables.alternate_filtering')
      ) {
        $loot = Item::where('item_category_id', $id)
          ->released()
          ->whereNotNull('data')
          ->where('data->rarity', $criteria, $rarity)
          ->get();
      } else {
        $loot = Item::where('item_category_id', $id)
          ->released()
          ->whereNotNull('data')
          ->whereRaw('JSON_EXTRACT(`data`, \'$.rarity\')' . $criteria . $rarity)
          ->get();
      }
    } else {
