...
class Feature extends Model {
  ...
  public function scopeSortCategory($query) {
    ...
    return count($ids)
      ? $query->orderByRaw(DB::raw('FIELD(feature_category_id, ' . implode(',', $ids) . ')'))
      : $query;
  }
  ...
  public function scopeSortSpecies($query) {
    ...
    return count($ids)
      ? $query->orderByRaw(DB::raw('FIELD(species_id, ' . implode(',', $ids) . ')'))
      : $query;
  }
  ...
  public function scopeSortRarity($query, $reverse = false) {
    $ids = Rarity::orderBy('sort', $reverse ? 'ASC' : 'DESC')
      ...
      ->toArray();
    return count($ids)
      ? $query->orderByRaw(DB::raw('FIELD(rarity_id, ' . implode(',', $ids) . ')'))
      : $query;
  }
  ...
  public function getDisplayNameAttribute() {
    return '<a href="' .
      ...
      $this->name .
      '</a>' .
      ($this->rarity ? ' (' . $this->rarity->displayName . ')' : '');
  }
