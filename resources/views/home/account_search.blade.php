                        ...
                        <ul>
                            ...
                            if (isset($item->trade_count) && $item->trade_count > 0) {
                              foreach ($tradesSent as $trade) {
                                if (
                                  isset($trade->data['sender']) &&
                                  $trade->data['sender'] != [] &&
                                  isset($trade->data['sender']['user_items']) &&
                                  isset($trade->data['sender']['user_items'][$item->id])
                                ) {
                                  $holdLocations['trade'][$trade->id] =
                                    $trade->data['sender']['user_items'][$item->id];
                                }
                              }
                              foreach ($tradesReceived as $trade) {
                                if (
                                  isset($trade->data['recipient']) &&
                                  $trade->data['recipient'] != [] &&
                                  isset($trade->data['recipient']['user_items']) &&
                                  isset($trade->data['recipient']['user_items'][$item->id])
                                ) {
                                  $holdLocations['trade'][$trade->id] =
                                    $trade->data['recipient']['user_items'][$item->id];
                                }
                              }
                            }
                            if (isset($item->update_count) && $item->update_count > 0) {
                              foreach ($designUpdates as $update) {
                                if (
                                  $update->data['user'] != [] &&
                                  isset($update->data['user']['user_items']) &&
                                  isset($update->data['user']['user_items'][$item->id])
                                ) {
                            ...
                            if (isset($item->submission_count) && $item->submission_count > 0) {
                              foreach ($submissions as $submission) {
                                if (
                                  ...
                                  isset($submission->data['user']['user_items']) &&
                                  isset($submission->data['user']['user_items'][$item->id])
                                ) {
                            ...
                            if (isset($holdLocations['trade'])) {
                              foreach ($holdLocations['trade'] as $trade => $quantity) {
                                array_push(
                                  $held,
                                  '<a href="' .
                                    App\Models\Trade::find($trade)->url .
                                    '">Trade #' .
                                    App\Models\Trade::find($trade)->id .
                                    '</a>' .
                                    ' (' .
                                    $quantity .
                                    ')'
                            ...
                            if (isset($holdLocations['update'])) {
                              foreach ($holdLocations['update'] as $update => $quantity) {
                                array_push(
                                  $held,
                                  '<a href="' .
                                    App\Models\Character\CharacterDesignUpdate::find($update)->url .
                                    '">Design Update #' .
                                    App\Models\Character\CharacterDesignUpdate::find($update)->id .
                                    '</a>' .
                                    ' (' .
                                    $quantity .
                                    ')'
                            ...
                            if (isset($holdLocations['submission'])) {
                              foreach ($holdLocations['submission'] as $submission => $quantity) {
                                array_push(
                                  $held,
                                  '<a href="' .
                                    App\Models\Submission\Submission::find($submission)->viewUrl .
                                    '">Submission #' .
                                    App\Models\Submission\Submission::find($submission)->id .
                                    '</a>' .
                                    ' (' .
                                    $quantity .
                                    ')'
