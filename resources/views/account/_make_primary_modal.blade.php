...
@else
  <p>This will make <strong>{!! $alias->displayAlias !!}</strong> your primary alias.
    Are you sure?</p>
  @if (!$alias->is_visible)
    <p class="text-danger">This alias is currently hidden from the public.
      Setting it as your primary alias will make it
      visible to everyone!</p>
  @endif
  {!! Form::open([
      'url' => 'account/make-primary/' . $alias->id,
      'class' => 'text-right'
  ]) !!}
