...
@section('admin-content')
  {!! breadcrumbs([
      'Admin Panel' => 'admin',
      ($isMyo ? 'MYO Approval' : 'Design Update') . ' Queue' => 'admin/designs/pending'
  ]) !!}
  ...
  <ul class="nav nav-tabs mb-3">
    <li class="nav-item">
      <a class="nav-link {{ set_active('admin/' . ($isMyo ? 'myo-approvals' : 'design-approvals') . '/pending*') }}"
        href="{{ url('admin/' . ($isMyo ? 'myo-approvals' : 'design-approvals') . '/pending') }}"
      >Pending</a>
    </li>
    <li class="nav-item">
      <a class="nav-link {{ set_active('admin/' . ($isMyo ? 'myo-approvals' : 'design-approvals') . '/approved*') }}"
        href="{{ url('admin/' . ($isMyo ? 'myo-approvals' : 'design-approvals') . '/approved') }}"
      >Approved</a>
    </li>
    <li class="nav-item">
      <a class="nav-link {{ set_active('admin/' . ($isMyo ? 'myo-approvals' : 'design-approvals') . '/rejected*') }}"
        href="{{ url('admin/' . ($isMyo ? 'myo-approvals' : 'design-approvals') . '/rejected') }}"
      >Rejected</a>
    </li>
  </ul>
    ...
    @foreach ($requests as $r)
      ...
      <div class="d-flex row flex-wrap col-12 mt-1 pt-1 px-0 ubt-top">
        ...
        <div class="col-4 col-md-1">
          <span
            class="btn btn-{{ $r->status == 'Pending' ? 'secondary' : ($r->status == 'Approved' ? 'success' : 'danger') }} btn-sm py-0 px-1"
          >{{ $r->status }}</span>
        </div>
        <div class="col-4 col-md-1">
