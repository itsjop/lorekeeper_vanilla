        ...
        @if (
          ...
          {!! Form::button(
              ...
              [
                  'class' =>
                      'btn ' .
                      ($submission->favorites->where('user_id', Auth::user()->id)->first() == null
                          ? 'btn-outline-primary'
                          : 'btn-primary'),
                  'data-toggle' => 'tooltip',
                  'title' =>
  ...
  <div class="mb-3 mb-sm-4">
    <div class="row">
      ...
      <div class="col-md text-right">
        ...
        Favorite{{ $submission->favorites->count() != 1 ? 's' : '' }} ・ {{ $commentCount }}
    ...
    <div class="col-md mb-4">
      <div class="row mb-4 no-gutters">
        <<<<<<<
          HEAD
          <div
          class="col-md-2 mb-4 mobile-hide text-center"
        >
          <a href="/user/{{ $submission->user->name }}">
            <img
              src="/images/avatars/{{ $submission->user->avatar }}"
      ...
      >>>>>>> de110d8d09ad768ace3543a9dec885e4d76e7915
      ...
      <div class="card mb-4">
        ...
        <div class="card-body">
          @if (
            ...
            <p>Check that your role in the collaboration is correct as listed, and if not, make any
              changes. You can also remove yourself from the collaborator list if necessary. When you
              are done, or if the record is already accurate, press "submit" to make any changes and
              mark yourself as having approved. You will be able to edit this until the submission is
              approved.</p>
            ...
            @foreach ($submission->collaborators as $collaborator)
              @if ($collaborator->user_id == Auth::user()->id)
                <div class="mb-2">
                  ...
                  {!! Form::label('remove_user', 'Remove Me', ['class' => 'form-check-label ml-3']) !!} {!! add_help(
                      'If toggled on, this will remove the record of your collaboration from this submission.'
                  ) !!}
      ...
      <div class="card mb-4">
        ...
        <div class="card-body">
          @foreach ($submission->promptSubmissions as $promptSubmission)
            <strong>
              <a href="{{ $promptSubmission->viewUrl }}">#{{ $promptSubmission->id }} for
                {!! $promptSubmission->prompt->name !!}</a>
  ...
  <script>
    $(document).ready(function() {
      $('.view-favorites').on('click', function(e) {
        e.preventDefault();
        loadModal("{{ url('gallery/view/favorites') }}/{{ $submission->id }}",
          'Favorited By');
      });
    });
  </script>
